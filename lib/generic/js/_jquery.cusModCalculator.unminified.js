 		...
 		base.init = function(){	
			...
			if(aModels.length != aDivs.length){
 			...
 			if(aModels.length >= 1){				 
 				...
 				//base.setCSS('components/nouislider/css/nouislider.css', 'main-nouislider-css');
 				//base.setCSS('components/nouislider/css/nouislider.pips.css', 'nouislider-pips-css');
 				//base.setCSS('components/nouislider/css/nouislider.tooltips.css', 'nouislider-tooltips-css');
						...
						if(v_accord != ''){
							...
							if(iButton == 0){ 
									...
									if(oHolder['position'] != 'foot'){								
										...
										v_str2 += '</div><div class="row"><div class="'+ v_diver +'">ddd&nbsp;</div><div class="'+ v_diver +'">'; 
		...
		base.setEvents = function(){
			...
			$(".obj-range").each(function(){
				...
				noUiSlider.create(aSliderInfo[v_thisConID]['objSlider'], {
					...
					start: [0],
					step: parseInt(v_thisCon.attr('data-slider-step')),
					connect: "lower",
					animate: false,  
					range: {
				...
				aSliderInfo[v_thisConID]['objSlider'].noUiSlider.on('update', function( values, handle ){					 
					...
					objInput.val  ( sTrFilter.replace('.00', '') );  	 
					objOutput.val( sTrFilter.replace('.00', '') ); 
		...
		base.emailFormButtons = function(arr){
			...
			$.each( arr['buttons'], function( i, v ){	
					...
					if(v['trigger'] == 'modal-close'){
						...
						strigger = ' class="btn btn-default" data-dismiss="modal" ';
